...
<script type="text/javascript">
	function check(time_id) {
		pwd = prompt('수정/삭제 하려면 비밀번호를 넣으세요');
		document.location.href="healthPlatform_control.jsp?action=timeEdit&time_id="+time_id+"&upasswd="+pwd;
	}
</script>
...
<div align="center"> 
   ...
   [<a href=time_form.jsp>운동시간 등록</a>]<p>
   

   
<form>
		<table border="1">
			<tr><th>번호</th><th>시작시간</th><th>운동시간</th><th>유저번호</th></tr>
			<%
				for(Time  hp : (ArrayList<Time>)datas) {
			%>
			  <tr>
			   <td><a href="javascript:check(<%=hp.getTime_id()%>)"><%=hp.getTime_id() %></a></td>
				<td><%=hp.getTime_start() %></td>
				<td><%=hp.getTime_duration() %></td>
				<td><%=hp.getUser_id() %></td>
			  </tr>
			 <%
				}
