...
public class HealthBean {
	...
	public ArrayList<User> searchUserDB(int isNum, int keyword) {
		connect();
		ArrayList<User> datas = new ArrayList<User>();
		
		String sql = "select * from user where user_name=? order by user_id desc";
		
		if (isNum == 0) {
			sql = "select * from user where user_name=? order by user_id desc";
		} else {
			sql = "select * from user where user_id=? order by user_id desc";
		}
